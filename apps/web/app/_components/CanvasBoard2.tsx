  ...
  useEffect(() => {
      ...
      switch (drawingShape) {
        ...
        case "diamond":
          ...
          shape = new fabric.Polygon(points, {
            ...
            strokeWidth: 3,
            objectCaching: false,
            originX: "center", // important
            originY: "center", // important
            selectable: false, // keep false while drawing
          });
      }
    ...
    const handleMouseMove = (opt: fabric.TEvent<fabric.TPointerEvent>) => {
      ...
      if (drawingShape === "diamond" && tempShape instanceof fabric.Polygon) {
        ...
        tempShape.set({
          left: (pointer.x + startPoint.current.x) / 2,
          top: (pointer.y + startPoint.current.y) / 2,
        });
      }
    ...
    const handleMouseUp = () => {
      ...
      if (tempShape) {
        if (drawingShape === "diamond" && tempShape instanceof fabric.Polygon) {
          // Make sure it's selectable and recalc bounding box
          tempShape.selectable = true;
          tempShape.evented = true;
          tempShape.setCoords(); // important!
        } else {
